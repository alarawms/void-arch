    ...
    for pkg in xbps-install pacman; do
        ...
        case $pkg in
            xbps-install)
                ...
                src_pkg_repo_install
                sudo xbps-install -S dmenu kitty xdotool polybar sxhkd nitrogen xwallpaper dwm slstatus st nsxiv font-fira-ttf font-firacode fzf font-awesome;
                cd $HOME/.local/
...
#############
desktop_file() {
    sudo bash -c 'cat > /usr/share/xsessions/herbstluftwm.desktop << EOF
[Desktop Entry]
Encoding=UTF-8
Name=herbstluftwm
Comment=berry - a small window manager
